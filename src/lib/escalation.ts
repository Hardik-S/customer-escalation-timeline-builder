...
const datePattern =
  /(?:by|before|no later than|due) (?:(?:EOD|end of day) )?(?<month>Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?) (?<day>\d{1,2})(?:st|nd|rd|th)?(?:(?: at |,? )(?<time>\d{1,2}:\d{2}))?/i;
